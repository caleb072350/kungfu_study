...
void IMDEngine::listening() {
    ...
    if (IEngine::signal_received >= 0) {
        KF_LOG_INFO(logger, "[IEngine] signal_received: " << IEngine::signal_received);
    }
...
void IMDEngine::on_market_data(const LFMarketDataField* data) {
    if (isRunning) {
        writer->write_frame(data, sizeof(LFMarketDataField), source_id, MSG_TYPE_LF_MD, 1/*is_last*/, -1/*invalidRid*/);
