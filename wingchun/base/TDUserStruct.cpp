...
{
    // force load read-only in this common constructor. only ITEngine may modify
...
void TDUserInfoHelper::clean_up(TDUserInfo* info)
{
    ...
    memset(info->orders, '\0', sizeof(TDOrderInfo) * AVAILABLE_ORDER_LIMIT);
}
...
inline bool order_is_existing(char status) {
    return status != ORDER_INFO_RAW && status != LF_CHAR_Error && status != LF_CHAR_AllTraded
...
void TDUserInfoHelper::set_pos(const string& user_name, const json& pos) {
    ...
    /** if loaded, simple skip internally */
    load(user_name);
...
{
    int idx = local_id % AVAILABLE_ORDER_LIMIT;
    int count = 0;
