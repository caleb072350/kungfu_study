/**
 * Handle position automated update.
*/
...
/**
 * easy macro for pos array index calculation
...
public:
    ...
    inline void add_pos(const string& ticker, LfPosiDirectionType dir, int tot, int yd)
    {
        ...
        pos_array[POS_ARRAY_IDX(dir, TOTAL_INDEX)] = pos_array[POS_ARRAY_IDX(dir, TOTAL_INDEX)].get<int>() + tot;
        pos_array[POS_ARRAY_IDX(dir, YESTD_INDEX)] = pos_array[POS_ARRAY_IDX(dir, YESTD_INDEX)].get<int>() + yd;
    }
    ...
    inline bool future_update(const string& ticker, int volume, LfDirectionType direction, LfOffsetFlagType offset)
    {
        ...
        switch (offset)
        {
            ...
            case LF_CHAR_LocalForceClose:
            {
                ...
                // then tol position needs to be revised.
                int tot = pos_array[POS_ARRAY_IDX(position_dir, TOTAL_INDEX)].get<int>() - volume;
                if (tot < pos_array[POS_ARRAY_IDX(position_dir, YESTD_INDEX)].get<int>() || tot < 0)
                    is_poisoned = true;
